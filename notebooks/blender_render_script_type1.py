...
parser.add_argument('--mesh_dir', type=str,
                    help='메시 파일들이 들어 있는 디렉터리 (지정하면 --object_path_pkl 무시)')
parser.add_argument('--output_dir', type=str, required=True,
                    help='렌더 결과를 저장할 최상위 디렉터리')

argv = sys.argv[sys.argv.index("--") + 1 :]
args = parser.parse_args(argv)

from pathlib import Path
SUPPORTED = {'.obj', '.glb', '.gltf', '.stl', '.fbx', '.ply'}

if args.mesh_dir:                       # 새 옵션이 있으면 폴더 스캔
    # uid_paths = [str(p) for p in Path(args.mesh_dir).rglob('*')
    #              if p.suffix.lower() in SUPPORTED]
    uid_paths = [args.mesh_dir]
    print(args.mesh_dir)
    print(uid_paths)
# random.shuffle(uids)
...
for uid in uid_paths:
    ...
    from pathlib import Path
    cur_path = os.path.join(args.output_dir, Path(uid).stem)
    print(f"curpath: {cur_path}")
